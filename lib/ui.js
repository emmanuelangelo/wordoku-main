...
//                console.dir(e);
            ...
            fetch("9plus-def.json").then(function(response) {
                return response.json();
            }).then(function(data) {
                app.config.definitions = data;
...
//            console.log("Attempting to find a good sudoku puzzle...");
            ...
            for (let i=0; i < (6 - app.config.difficultyLevel.indexOf(app.state.difficulty)); i++) {
            ...
            app.storeState('state', app.state);
            app.showDefinitions(app.state.wordStr);
        },
        ...
        showDefinitions: function(word) {
            const defs = app.config.definitions[word.toLocaleLowerCase()];
            let out = "<div class='def'>Definition: ";

            for (let i=0; i<defs.length; i++) {
                out += "<span class='definition'>" + (i + 1) + ". (<em>" + defs[i].partOfSpeech + "</em>) " + defs[i].definition + "</span>";
            }
            out += "</div>";
            $("#definition").innerHTML = out;
        },
...
//                        $(item.id + " span.num").style.transform = 'scale(1.5)';
        ...
        showAbout: function() {
            ...
            out += "<p>By GM &lt;<a href='mailto:chinonsomaduka2000'>Nova.com</a>&gt;</p>";
            out += "<p>&copy; Copyright 2022, All Rights Reserved</p>";
            $("#about").innerHTML = out;
